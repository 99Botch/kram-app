    ...
    <div class="hld-profile" v-if="!loading">
        ...
        <div class="profile-forms">
            ...
            <div class="forms forms-sub-change">
                ...
                >
                <el-button class="warning-del" @click="deleteBox = true"
...
export default {
    ...
    methods: {
        ...
        updUrlDB(_results) {
            ...
            axios
                .put(`${URI}/users/pic/${this.id}`, json, {
                    headers: {
        ...
        async submitProfile() {
            ...
            if (this.user.username.length == 0 || this.user.email.length == 0)
                ...
                this.profileError = "Username must be 6 characters minimum";
            else if (!validator.validate(this.user.email))
                this.profileError =
                    "Ensure that you entered correct email format";
            else if (
                this.user.username.length > 6 &&
                validator.validate(this.user.email)
            ) {
                axios
                    .put(`${URI}/users/${this.id}`, json, {
                        headers: {
                            authorization: `Bearer ${localStorage.getItem(
                                "token"
                            )}`,
        ...
        checkPass() {
            const REG_PATTERN =
                "^(?=.*[0-9])(?=.*[a-zA-ZÀ-ȕ])([a-zA-ZÀ-ȕ0-9(),-_.,@ ]+)$";
            !this.user.new_password.match(REG_PATTERN)
                ? (this.passError =
                      "Password must be 8 characters long, witn a one lowercase/uppercase character and a number")
                : (this.passError = "");
        },

        checkRePass() {
            this.user.new_password !== this.user.repeat_password
                ? (this.passError = "Both passwords must match!")
                : (this.passError = "");
        },
        ...
        submitPass() {
            ...
            if (this.user.username.length == 0 || this.user.email.length == 0)
                ...
                axios
                    .put(`${URI}/users/password/${this.id}`, json, {
                        headers: {
                            authorization: `Bearer ${localStorage.getItem(
                                "token"
                            )}`,
                    ...
                    .catch((error) => {
                        if (
                            error.response.data ===
                            "Old password is not matching database"
                        )
                            this.passError = "Error: wrong password";
                        if (error.response.data.type === "string.pattern.base")
                            this.passError =
                                "Password must be 6 characters long, with minimum one lowercase & uppercase character and a number";
                        if (error.response.data.type === "any.required")
                            this.passError = "Fill in the form !";
                    });
            }
        },
    },
