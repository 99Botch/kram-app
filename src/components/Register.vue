        ...
        <div class="form-modal" id="registerBox">
            ...
            <div>
                ...
                <div class="form-modal-btn">
                    <p class="form-error" id="registerFormError">{{ this.formError }}</p>
                    <button class="submit" id="submitRegister" type="submit" @click="submitForm" disabled>Register</button>
                </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // FORM VALIDATION
            validation(){
                if (document.getElementById("emailRegister").value.length !=0 && document.getElementById("usernameRegister").value.length !=0 && 
                document.getElementById("passwordRegister").value.length !=0 && document.getElementById("rePassRegister").value.length !=0 ) {
            ...
            mailValidation(){
                let [length, format, error, warning, initial] = [document.getElementById("emailRegister").value.length, 
            ...
            usernameValidation(){
                let [length, error, warning, initial] = [document.getElementById("usernameRegister").value.length,
                    document.getElementById("registerUsernameError"),
                    '#FFA500',
                    '#DB3C3A'];

                (length == 0) ? (this.usernameError = "Enter a username", error.style.color = initial) : 
                    (length < 6) ? (this.usernameError = "Username must be at least 6 characters", error.style.color = warning) : 
                    (length > 99) ? (this.usernameError = "Username cannot exceed 99 characters", error.style.color = warning) : 
                    this.usernameError = "";
            },
            passValidation(){
                let [length, error, warning, initial] = [document.getElementById("passwordRegister").value.length,
                    document.getElementById("registerPassError"),
                    '#FFA500',
                    '#DB3C3A'];

                (length == 0) ? (this.passError = "Enter a password", error.style.color = initial) : 
                    (length < 8) ? (this.passError = "Password must be at least 8 characters", error.style.color = warning) : 
                    this.passError = "";
            },
            rePassValidation(){
                let [error, main, copy, warning, initial] = [document.getElementById("registerRePassError"),
                    this.form.password,
                    this.form.repeat_password,
                    '#FFA500',
                    '#DB3C3A'];

                (main !== copy) ? (this.rePassError = "Password and Confirm password must be match", error.style.color = warning) : (this.rePassError = "", error.style.color = initial);
            }
        }
    }
</script>
