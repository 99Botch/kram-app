<template>
        <div class="pop-menu" @click="outsider" id="popMenu">
            <div class="pop-menu-holder" :class="(windowWidth >= 600) ? 'two-sided' : null">
                ...
                <div>
                    ...
                    <div class="pop-update-form">
                        <div>
                            ...
                            <el-input class="input-inner" type="text" name="" v-model="form.name" required placeholder="Name" id="deckName"/>
                        </div>
                        ...
                        <div>
                            ...
                            <el-input v-model="form.description" id="deckDescription" type="textarea" />
                        </div>
                        <p :class="(this.descriptionErr) ? 'error-msg' :  'error-msg-empty' ">{{ this.descriptionErr }}</p>

                        <div class="toggle-privacy">
                            <label for="privacy">Deck privacy</label>
                            <input type="checkbox" id="privacy" v-model="form.private" @click="this.privacy = !this.privacy"/>
                        </div>
                    </div>
                    
                    <div class="btns-modal-form pop-update-foot">
                        <button type="button" @click="$emit('clicked', false)">Close</button>
                        <button type="submit" @click="updateDeck">Update deck</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        methods: {
            ...
            async updateDeck(){
                if(this.form.name.length < 6){
                    ...
                    this.descriptionErr = "Enter at least 10 characters to set it public"
                } else {
                    this.form.private = this.privacy;
                    if(this.form.description.length <1) this.form.description = null;
...
<style lang="scss">
    ...
    .pop-update-form{
        ...
        .el-input__inner, .el-textarea textarea{
            border-radius: 0px;
            box-shadow: none !important;
            border: 1.5px solid #DDD;
                
            &:focus, &:hover, .el-select:hover:not(.el-select--disabled) .el-input__inner{
                box-shadow: none !important;
